...
export class SolidStarService {
  ...
  generateCanvas(ctx, paths, solid: any[], solidSettingsService?, colorManagerService?, test: boolean = false){
    ...
    const getCoord = (coordinate: Coordinate) => {
      ...
      return {
        ...
        z: round(project(coordinate.z) + (solidSettingsService.definition / 2)),
    ...
    if (test){
      let to_color = (c)=> ('0'+Math.round(Math.max(0, Math.min(1,(c+170)/340))*256).toString(16)).slice(-2);
      colorManagerService.storedColors = [];
      console.log('test')

      for(let color of solid.map(c => '#'+ to_color(c.ac.x)+to_color(c.ac.y)+to_color(c.ac.z))){
      ...
      colorManagerService.inputColors = {...colorManagerService.storedColors};
